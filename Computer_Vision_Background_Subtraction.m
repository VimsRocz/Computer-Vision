...
for i = 2:length(file_list)
    ...
    % Output
    subplot(231); 
    imagesc(im_RGB, [0 255]); % TODO: Show input image   we can write or im or im_RGB
    subplot(232); 
    imagesc(mu, [0 255]); % TODO: Show mean values
    subplot(233);  
    imagesc(sigma_square, [0 255]); % TODO: Show variances
    subplot(234);  
    imagesc(del_g, [0 255]); % TODO: Show difference between current image and mean values
    subplot(235);  
    imagesc(before_mask, [0 1]); % TODO: Show background mask (unfiltered)
    subplot(236);  
    imagesc(return_mask, [0 1]); % TODO: Show background mask after eliminating noise
    drawnow; 
end